...
// GET SEARCH PARAMS FROM FE SEARCH BAR
export async function POST(NextRequest: NextRequest) {
    try {
        const searchRequest = await NextRequest.json()

        search = searchRequest.search

        if (!search) {
            NextResponse.json({
                message: "search not founded",
                status: 408
            })
        }

        return NextResponse.json({ message: 'status founded', status: 200 })
...
export async function GET() {
    ...
    try {
        ...
        // RETURN STORES DATA
        return NextResponse.json({ storesInfo, status: 200 });
